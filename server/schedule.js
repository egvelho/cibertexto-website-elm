...
const server = express(); server.use(bodyParser.json());
...
function getAvailableHours(date, storage) {
  ...
  const hours = [ 10, 11, 14, 16, 18 ].filter((hour) =>
    orders.indexOf(hour) === -1
  )
...
function validateOrder(order) {
  ...
  const valid =
    ...
    ([ 'format',
       ...
       'help',
       'lawyer',
       'development' ].indexOf(option) > -1) &&
    ([ 0, 1, 2 ].indexOf(problem) > -1) &&
    ([ 0, 1 ].indexOf(backupOption) > -1) &&
    (name.length > 2 && name.length < 36) &&
    (obs.length < 128) &&
    (place.length > 4 && place.length < 32) &&
    (address.length > 8 && address.length < 48);
  ...
  switch(option) {
    ...
    case 'startupProblem':
      const basePrice = urgency ? 120.00 : 80.00;
      delivery = urgency ? 3 : 7;
      if (problem === 2) {
        price = basePrice + 300.00;
        detail = 2;
      } else if (problem === 1) {
...
function getId(storage) {
  ...
  const exists = storage.filter((order) =>
    order.id === id
  ).length > 0 ? true : false;
...
server.post('/', (req, res, next) => {
  req.webtaskContext.storage.get((error, storage) => {
    if (error) {
    ...
    if (typeof order === 'undefined') {
      ...
      req.webtaskContext.storage.set(storage.concat(order), error => {
        if (error) {
          res.status(200).send({ status: 'error', order: '' });
        }
...
server.get('/', (req, res, next) => {
  req.webtaskContext.storage.get((error, storage) => {
    ...
    const schedule =
      getNext15Days(undefined, undefined, storage)
      .map((date) => parseDate(date, storage));
